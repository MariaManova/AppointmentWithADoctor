...
const DiagnosisCard = ({ data, onPress }: any) => {
    const { indicator, sub } = globalStyles
    const { container, images, h1, dateStyle, iconStyle } = styles
    const { dateTimeReceipt, user, uid, diagnosis, recommendations, editedAt } = data
    const { speciality, photo, fullName } = user
    return (
        <TouchableOpacity >
