...
import { Patient, initPatient, PatientCards } from '../../interfaces'
...
import { DOCTORProfile, CHAT, CHATTWO } from '../../routes';
import { ChatCard } from '../ProfileTab/ChatCard';
import { DoctorACardTwo } from '../ProfileTab/DoctorACardTwo';
import { ChatTwo } from './ChatTwo';
import { DiagnosisCard } from '../ProfileTab/DiagnosisCard';
...
interface ProfileData {  
  patient: Patient,
  history: PatientCards[],
}
var data: ProfileData = {
  patient: initPatient,  history: []
}

class ListScreenPatientCard extends React.Component<any, State> {
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      if (param) id = param;
      if (token) {
        const response = await fetch(serverUrl + 'patientcard/' + id,
          {
        ...
        if (response.status == 200) {
          ...
          console.log('Успех fetch ' + logAction, 'patient: '+data.patient+', history: '+data.history)
        }
  ...
  render() {
    ...
    return (
      <View >
        ...
        <Header title='Карточка пациента'
          leftIcon={'arrow-back'}
  ...
  private myChats() {
    ...
    return (
      ...
      <List.Accordion
        title={"История болезней (" + (history.length ? history.length : 0)+')'}
        left={props => <List.Icon {...props} icon="history" />}
        style={sectionContainer1}
        titleStyle={sectionTitle1}
      >
      {history.map((item: PatientCards, id) => {
            return <DiagnosisCard data={item} key={id} 
           />
          })}
      </List.Accordion>
    </List.Section>
