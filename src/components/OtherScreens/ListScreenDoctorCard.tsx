...
import { Patient, initPatient, DoctorChat, InitialDoctor, Doctor, PatientCards } from '../../interfaces'
...
import { ChatCardTwo } from '../ProfileTab/ChatCardTwo';
import { DiagnosisCard } from '../ProfileTab/DiagnosisCard';
import { DiagnosisCardTwo } from '../ProfileTab/DiagnosisCardTwo';
...
class ListScreenDoctorCard extends React.Component<any, State> {
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      if (param) id = param;
      if (token) {
        const response = await fetch(serverUrl + 'patientcard?id_Doctor='  + userLogin.id,
          {
  ...
  private myChats() {
    ...
    return (
      ...
      <List.Accordion
        title={"История назначений (" + (history.length ? history.length : 0)+')'}
        left={props => <List.Icon {...props} icon="history" />}
        style={sectionContainer1}
        titleStyle={sectionTitle1}
      >
      {history.map((item: PatientCards, id) => {
           return <DiagnosisCardTwo data={item} key={id} 
