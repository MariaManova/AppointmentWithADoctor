...
import {  DOCTORProfile, DOCTORProfileTwo } from '../../routes';
...
class ListDoctorsScreenTwo extends React.PureComponent<any, State> {
          ...
          <View>
            {data.map(item => {
              return <DoctorCard data={item} key={item.id}
                onPress={() => navigation.navigate(DOCTORProfileTwo, (item))} />//
            })}
          </View> 
          :
          !loadError &&
